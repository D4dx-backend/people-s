...
import AttachmentsUploader, { type Attachment } from "@/components/programreport/AttachmentsUploader";
...
export default function ProgramReportFill() {
  ...
  // Attachments state — kept in sync with the server-side submission document
  const [attachments, setAttachments] = useState<Attachment[]>([]);

  // ── Load form config + existing draft/submission ──────────────────────────
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.data?.hasConfiguration || !res.data?.formConfiguration?.isPublished) {
          setError("This program report form is not available.");
          return;
        }
        ...
        // Check for an already-submitted submission
        try {
          const subRes = await programReports.getSubmissions(reportId, { status: "submitted" });
          const submittedSub = (subRes.data?.submissions || []).find((s: any) => s.status === "submitted");
          if (submittedSub) {
            setViewMode(true);
            setViewedSubmission(submittedSub);
            setFormData(submittedSub.formData || {});
            setSubmissionId(submittedSub._id);
            setAttachments(submittedSub.attachments || []);
            setLoading(false);
            return;
          }
        ...
        // Load existing draft
        try {
          const subRes = await programReports.getSubmissions(reportId, { status: "draft" });
          ...
          if (myDraft) {
            ...
            setAttachments(myDraft.attachments || []);
  ...
  // ── Ensure a draft exists (for attachment upload) ─────────────────────────

  const ensureDraft = async (): Promise<string | null> => {
    if (submissionId) return submissionId;
    if (!reportId) return null;
    try {
      const res = await programReports.saveDraft(reportId, { formData });
      const sid = res.data?.submission?._id;
      if (sid) { setSubmissionId(sid); return sid; }
      return null;
    } catch {
      toast({ title: "Could not create draft. Save your draft first.", variant: "destructive" });
      return null;
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing && submissionId) {
        await programReports.updateSubmission(reportId, submissionId, { formData });
        ...
        if (!sid) {
          const draftRes = await programReports.saveDraft(reportId, { formData });
          sid = draftRes.data?.submission?._id;
          if (sid) setSubmissionId(sid);
        }
        if (!sid) throw new Error("Could not create submission");
        await programReports.submitForm(reportId, sid, { formData });
        setSubmitted(true);
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await programReports.deleteSubmission(reportId, submissionId);
  ...
  if (viewMode && formConfig) {
    ...
    return (
      <div className="container max-w-2xl mx-auto py-6 space-y-6">
        <Button variant="ghost" size="sm" onClick={() => navigate("/program-reports")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Back to Program Reports
        </Button>
          ...
          <CardHeader className="pb-3">
            ...
            <div className="flex gap-2 mt-3">
              <Button
                size="sm"
                variant="outline"
                onClick={() => { setViewMode(false); setIsEditing(true); setCurrentPage(0); }}
              >
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {attachments.map((att) => (
                  ...
                  >
                    {att.kind === "image" ? (
                      <img src={att.url} alt={att.fileName} className="w-full h-20 object-cover rounded mb-1" />
                    ) : (
                      <div className="w-full h-20 flex items-center justify-center bg-muted rounded mb-1 text-muted-foreground">
                        <Paperclip className="h-6 w-6" />
                      </div>
                    )}
                    <p className="text-xs text-center truncate w-full text-muted-foreground">{att.fileName}</p>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete your submission for <strong>{formConfig.title}</strong>?
                This will also remove all uploaded attachments. This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
  ...
  if (submitted) {
    return (
      <div className="container max-w-2xl mx-auto py-16 text-center space-y-4">
        ...
        <h2 className="text-xl font-semibold">Program Report Submitted Successfully!</h2>
        <p className="text-muted-foreground text-sm">
          {formConfig.submissionSettings?.confirmationMessage || "Thank you for submitting the program report."}
        </p>
        <Button onClick={() => navigate("/program-reports")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Back to Program Reports
        </Button>
      </div>
    );
  ...
  return (
    <div className="container max-w-2xl mx-auto py-6 space-y-6">
      ...
      {/* Attachments section — shown on last page */}
      {isLastPage && (
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center gap-2">
            ...
            <CardDescription>
              Upload photos, videos, or PDF documents for this program report.
          ...
          <CardContent>
            {submissionId ? (
              ...
              <div className="text-sm text-muted-foreground space-y-2">
                <p>Save a draft first to enable file uploads.</p>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={async () => {
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {isEditing && (
          <Button variant="outline" onClick={() => { setIsEditing(false); setViewMode(true); setCurrentPage(0); }}>
            Cancel
          </Button>
        )}
